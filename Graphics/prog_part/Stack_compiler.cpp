...
void compile_prog(char* my_buff, char* comp_my_buf, long int size_of_prog)
{
    ...
    while(cur_poz != size_of_prog && my_buff[cur_poz] != -1)
    {
...
void s_n_t_sub(char* my_buff, long int size_of_prog)
{
	...
	printf("size_of_prog = %ld\n", size_of_prog);

    while(cur_poz != size_of_prog && my_buff[cur_poz] != '\0')
    {
        if(isalpha(my_buff[cur_poz]) == 0 && isdigit(my_buff[cur_poz]) == 0 && my_buff[cur_poz] != ':')
        {
            my_buff[cur_poz] = '\0';
        }
        cur_poz++;
